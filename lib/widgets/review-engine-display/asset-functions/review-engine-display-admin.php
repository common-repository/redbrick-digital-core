...
<div class="rbd-core-ui">
	...
	<div data-grid="grid" data-columns="2">
		...
		<label>
			...
			<select class="widefat" id="<?= $this->get_field_id( 'columns' ); ?>" name="<?= $this->get_field_name( 'columns' ); ?>">
				<?php
					for( $i = 1; $i <= 4; ++$i ){
						$selected = ( $columns == $i ) ? 'selected' : '';
	...
	<div data-grid="grid" data-columns="3">
		...
		<label>
			...
			<input class="widefat" id="<?= $this->get_field_id( 'characters' ); ?>" name="<?= $this->get_field_name( 'characters' ); ?>" type="number" value="<?= esc_attr( $characters ); ?>" />
		</label>
		<?php
	...
	<div data-grid="grid" data-columns="2">
		...
		<div>
			<input style="position:relative;top:3px;left:5px;" type="checkbox" id="<?= $this->get_field_id( 'disable_3d' ); ?>" name="<?= $this->get_field_name( 'disable_3d' ); ?>" value="true" <?php if( $disable_3d == true ){ echo 'checked'; } ?> />
			<label for="<?= $this->get_field_id( 'disable_3d' ); ?>"><span>Disable 3D?</span></label>
		</div>
	</div>
